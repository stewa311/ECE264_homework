...
int countVector(char * filename)
{
  ...
  Vector * temp; // temporary vector for while loop
  
  fseek(fptr, 0, SEEK_SET); // returns to the start of stream
  
  int count = 0;
  
  while (fread(& temp, sizeof(Vector), 1, fptr))
  {
    count++;
  }
